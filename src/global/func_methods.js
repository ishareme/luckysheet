...
function getInlineStringText(cell){
    if(
        cell &&
        cell.ct != null &&
        cell.ct.t === "inlineStr" &&
        ...
        let text = "";
        for(let i = 0; i < cell.ct.s.length; i++){
            let item = cell.ct.s[i];
            if(item == null){
                continue;
            }
...
function getCellPrimitiveValue(cell){
    ...
    if(value == null){
        const inlineValue = getInlineStringText(cell);
        if(inlineValue != null){
            return inlineValue;
        }
    }

    if(value == null && cell.m != null){
        return cell.m;
    }
...
const func_methods = {
    ...
    getCellDataArr: function(rangeObj, nullCellType, isNeglectNullCell){
        ...
        if(rangeObj.data == null){
            ...
            if(getObjType(rangeObj.data) === "array"){
                ...
                let singleValue = getCellPrimitiveValue(rangeObj.data);
                if(singleValue == null || isRealNull(singleValue)){
                    if(!isNeglectNullCell){
                        if(nullCellType == "number"){
                            dataArr.push(0);
                        }
        ...
        if(getObjType(data) == "array"){
            ...
            if(data.data == null){
                ...
                if(data.rowl == 1 && data.coll == 1){
                    ...
                    let primitiveValue = getCellPrimitiveValue(cellData);

                    if(cellData == null || isRealNull(primitiveValue)){
                        if(nullCellType == "number"){
                            value = 0;
                        }
                        else if(nullCellType == "text"){
                    ...
                    if(data.data[0][0].mc != null && data.data[0][0].mc.rs == data.rowl && data.data[0][0].mc.cs == data.coll){
                        let cellValue = data.data[0][0];
                        let primitiveValue = getCellPrimitiveValue(cellValue);

                        if(cellValue == null || isRealNull(primitiveValue)){
                            if(nullCellType == "number"){
                                value = 0;
                            }
                            else if(nullCellType == "text"){
                        ...
                        if(data.rowl == 1){
                            ...
                            let cellValue = data.data[0][cell_c - stc];
                            let primitiveValue = getCellPrimitiveValue(cellValue);

                            if(cellValue == null || isRealNull(primitiveValue) || (cellValue && cellValue.mc != null)){
                                if(nullCellType == "number"){
                                    value = 0;
                                }
                                else if(nullCellType == "text"){
                            ...
                            let cellValue = data.data[cell_r - str][0];
                            let primitiveValue = getCellPrimitiveValue(cellValue);
    ...
    getCellDate: function(data){
        ...
        if(isRealNum(date_text)){
            date_text = update("YYYY-MM-DD h:mm:ss", parseFloat(date_text));
        }
