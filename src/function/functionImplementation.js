...
const functionImplementation = {
    ...
    "EDATE": function() {
        ...
        try {
            ...
            //计算
            var edate = dayjs(start_date).add(months, 'months');

            if (!edate.isValid()) {
                return formula.error.v;
            }

            var baseSerial = datenum_local(edate.toDate());
            if (!isRealNum(baseSerial)) {
                return formula.error.v;
            }

            var mask = genarate(baseSerial);

            return mask != null ? mask[2] : baseSerial;
        }
        catch (e) {
            var err = e;
            err = formula.errorInfo(err);
