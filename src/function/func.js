...
function inlineSegmentToPlainText(segment){
    if(segment == null){
        return "";
    }
...
function getInlineStringValue(cell){
    if(
        cell &&
        cell.ct &&
        cell.ct.t === "inlineStr" &&
        ...
        for(let i = 0; i < cell.ct.s.length; i++){
            text += inlineSegmentToPlainText(cell.ct.s[i]);
        }
...
function normalizeInlineStringCell(cell){
    if(cell == null || getObjType(cell) !== "object"){
        return cell;
    }

    const inlineText = getInlineStringValue(cell);
    if(inlineText == null){
        return cell;
    }
...
function getPlainTextFromCellData(cell){
    if(cell == null || getObjType(cell) !== "object"){
        return cell;
    }
    ...
    function booleanOperation(a, operator, b){
        ...
        if(operator == "=="){
            if(isBlankValue(a) || isBlankValue(b)){
                return isBlankValue(a) && isBlankValue(b);
            }
...
function luckysheet_getcelldata(txt) {
    ...
    if (rangetxt.indexOf(":") == -1) {
        ...
        if (!isNaN(row) && !isNaN(col)) {
            ...
            ret = normalizeInlineStringCell(ret);
        ...
        if(ret != null){
            for(let rIndex = 0; rIndex < ret.length; rIndex++){
                if(ret[rIndex] == null){
                    continue;
                }

                for(let cIndex = 0; cIndex < ret[rIndex].length; cIndex++){
                    ret[rIndex][cIndex] = normalizeInlineStringCell(ret[rIndex][cIndex]);
                }
            }
        }
    ...
    if(typeof value === "object" ){
        if(value == null){
            ...
            if(Array.isArray(value.data)){ //单元格区域
                ...
                return getPlainTextFromCellData(value.data);
            }
        }
    }
        ...
        if(typeof value === "object" ){
            if(value == null){
                ...
                if(Array.isArray(value.data)){ //单元格区域
                    ...
                    let plainText = getPlainTextFromCellData(value.data);
                    if(plainText == null){
                        plainText = "";
                    }
                    value = plainText;
